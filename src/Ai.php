...
class Ai
{
  ...
  */
    ...
    /**
     * 模糊图片检测		判断一个图像的模糊程度
     * @param string $image 原始图片的base64编码数据（原始图片的base64编码数据（大小上限500KB），支持jpg、png、bmp，尺寸长宽不超过1080）
     * @return boolean|array url 成功则返回检测后的结果
     */
    public function image_fuzzy($image){
        $this->values['app_id'] = $this->appid;
        $this->values['time_stamp'] = time();
        $this->values['nonce_str'] = self::generateNonceStr();
        $this->values['image'] = urlencode($image);
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::IMAGE_FUZZY,$data);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['ret'])) {
     ...
     * @param string $image 原始图片的base64编码数据（原始图片的base64编码数据（大小上限500KB），支持jpg、png、bmp，尺寸长宽不超过1080）
     * @return boolean|array url 成功则返回识别后的结果
     */
    public function image_food($image){
        $this->values['app_id'] = $this->appid;
        $this->values['time_stamp'] = time();
        $this->values['nonce_str'] = self::generateNonceStr();
        $this->values['image'] = urlencode($image);
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::IMAGE_FOOD,$data);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['ret'])) {
    ...
    public function image_tag($image){
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::IMAGE_TAG,$data);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['ret'])) {
     ...
     * 人脸检测	识别上传图像上面的人脸信息。
     * @param string $image 原始图片的base64编码数据（原始图片的base64编码数据（大小上限500KB），支持jpg、png、bmp，尺寸长宽不超过1080）
     * @param int $mode 原始图片的base64编码数据（原始图片的base64编码数据（大小上限500KB），支持jpg、png、bmp，尺寸长宽不超过1080）
     * @return boolean|array url 成功则返回识别后的结果
     */
    public function face_detectface($mode = 1,$image){
        $this->values['app_id'] = $this->appid;
        $this->values['time_stamp'] = time();
        $this->values['nonce_str'] = self::generateNonceStr();
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::FACE_DETECTFACE,$data);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['ret'])) {
    ...
    /**
     * 人脸对比	对请求图片进行人脸对比。
     * @param string $image_a 	待对比人脸图片A,原始图片的base64编码数据（解码后大小上限1MB，支持JPG、PNG、BMP格式）
     * @param string $image_b 待对比人脸图片B,原始图片的base64编码数据（解码后大小上限1MB，支持JPG、PNG、BMP格式）
     * @return boolean|array url 成功则返回对比后的结果
     */
    public function face_facecompare($image_a,$image_b){
        $this->values['app_id'] = $this->appid;
        $this->values['time_stamp'] = time();
        $this->values['nonce_str'] = self::generateNonceStr();
        $this->values['image_a'] = urlencode($image_a);
        $this->values['image_b'] = urlencode($image_b);
        $this->setSign();//签名

        $data =  $this->toUrlParams();

        $result = $this->http_post(self::API_URL_PREFIX.self::FACE_FACECOMPARE,$data);
